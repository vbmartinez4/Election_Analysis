...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if county_name not in county_names:
            ...
            # Challenge-4b: Add the existing county to the list of counties.
            county_names.append(county_name)

            # Challenge-4c: Begin tracking the county's vote count.
...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        # Challenge-6b: Retrieve the county vote count.
        county_vote = county_votes.get(county_name)
        # Challenge-6c: Calculate the percentage of votes for the county.
        county_percent = float(county_vote) / float(total_votes) * 100
         # Challenge-6d: Print the county results to the terminal.
        county_results = (
            f"{county_name}: {county_percent:.1f}% ({county_vote:,})\n")
        print(county_results)
         # Challenge-6e: Save the county votes to a text file.
        txt_file.write(county_results)

         # Challenge-6f: Write an if statement to determine the winning county and get its vote count.
